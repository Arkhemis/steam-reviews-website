...
// « Battle 3 » : la même paire de jeux, réglée en duel au tour par tour. Le
...
export async function generateMetadata({ searchParams }: Battle3PageProps): Promise<Metadata> {
  ...
  const url = battleHref(leftAppId, rightAppId, "/battle-3");
  return {
    title,
    description,
    ...
    return (
      <div className="min-h-screen bg-[#0c1116] text-[#eef2f4]">
        ...
        <div className="mx-auto max-w-[640px] px-6 py-16 text-center">
          ...
          <Link href="/battle-3" className="mt-6 inline-block rounded-full bg-brand-blue px-[18px] py-2.5 text-sm font-bold text-[#0c1116]">
            Back to the duel
          </Link>
        </div>
        <BattleRivalries leftAppId={leftAppId} rightAppId={rightAppId} path="/battle-3" />
      </div>
    );
  }
      ...
      <div className="border-b border-[#1a2530] bg-[linear-gradient(115deg,#0d2018_0%,#0c1116_50%,#2a1206_100%)] px-5 py-8 sm:px-8">
        <div className="mx-auto max-w-[1320px]">
          <div className="flex flex-wrap items-baseline justify-between gap-3">
            <p className="font-mono text-[11px] tracking-[0.16em] text-brand-blue uppercase">battle 3 · duel mode</p>
            <div className="flex gap-4 font-mono text-[10px] tracking-[0.12em] text-[#9fb2bd] uppercase">
              <Link href={battleHref(leftAppId, rightAppId)} className="hover:text-brand-blue">
                classic battle →
              </Link>
              <Link href={battleHref(leftAppId, rightAppId, "/battle-2")} className="hover:text-brand-blue">
                army battle →
              </Link>
            </div>
          </div>
